...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      options: {
         ...
         buttonText : {
            today : "today",
            prevDay : "<i class=\"icon-chevron-left\"></i>",
            nextDay : "<i class=\"icon-chevron-right\"></i>"
         },
      ...
      /*
       * center the calendar to a middle position
       */
      center: function() {
         var events = this.options.data.events;
         var center_event = events[Math.floor(events.length/2)];
         var new_date = new Date(center_event.start - (24*60*60*1000)*3);
         var old_date = this.element.data("startDate");
         var day_diff = Math.floor((new_date - old_date) / (24*60*60*1000));
         this.element.data("startDate", new_date);
         this.options.firstDayOfWeek = (this.options.firstDayOfWeek + day_diff + 7) % 7;
         this._clearCalendar();
         this._loadCalEvents(new_date);
         this._scrollToHour(new_date.getHours());
      },
      /*
       ...
       */
      prevDay : function() {
         this.options.firstDayOfWeek = (this.options.firstDayOfWeek + 6) % 7 ;  
...
//            var calendarHeight = options.height(this.element);
			var calendarHeight = $('#event-calendar').height();
            var headerHeight = this.element.find(".wc-header").outerHeight();
            this.element.find(".wc-scrollable-grid").height(calendarHeight - headerHeight);
...
//            this.element.find(".wc-scrollable-grid").height(250);
         }
      },
      ...
      _setupEventDelegation : function() {
         ...
         this.element.click(function(event) {
            ...
            if ($target.hasClass("wc-cal-event")) {
               ...
               options.eventMouseover($target.parent().data("calEvent"), $target.parent(), event);
            }
            ...
            if ($target.hasClass("wc-cal-event")) {
               ...
               options.eventMouseout($target.parent().data("calEvent"), $target.parent(), event);
            }
         });
      },
...
//                     <button class=\"wc-prev\">" + options.buttonText.prevDay + "</button>\
//                     <button class=\"wc-next\">" + options.buttonText.nextDay + "</button>\
//                     </div>";
// 
...
//          }
         ...
         calendarHeaderHtml = "<table class=\"wc-header\"><tbody><tr><td class=\"wc-time-column-header wc-nav\"><button class=\"btn btn-mini wc-prev\">" + options.buttonText.prevDay + "</button></td>";
         for (var i = 1; i <= options.daysToShow; i++) {
            calendarHeaderHtml += "<td class=\"wc-day-column-header wc-day-" + i + "\"></td>";
         }
         calendarHeaderHtml += "<td class=\"wc-scrollbar-shim wc-nav\"><button class=\"btn btn-mini wc-next\">" + options.buttonText.nextDay + "</button></td></tr></tbody></table>";
         ...
         calendarBodyHtml += "</div></div></td>" + (self.options.daysToShow == 1 ? "" : "<td class=\"wc-dummy-column\"></td>") + "</tr><tr><td class=\"wc-grid-timeslot-header\">";
         ...
         calendarBodyHtml += (self.options.daysToShow == 1 ? "" : "<td class=\"wc-dummy-column\"></td>") + "</tr></tbody></table></div>";
...
//          });
      ...
      _renderEvents : function (events, $weekDayColumns) {
         ...
         $.each(eventsToRender, function(i, calEvent) {
			self._renderEvent(calEvent, $weekDayColumns);
			if (!options.readonly) {
		       self._addResizableToCalEvent(calEvent);
		    }
         });
      ...
      /*
       * Render a specific event into the calendar
       */
      _renderEvent: function (calEvent, $weekDayColumns) {
         var self = this;
         var options = this.options;
         ...
         var isSmallInterval = function() {
            return (calEvent.end.getTime() - calEvent.start.getTime()) <= 60 * 60 * 1000 / self.options.timeslotsPerHour;
         };

         var $weekDays = self._findWeekDaysForEvent(calEvent, $weekDayColumns);
         ...
         $weekDays.forEach(function($weekDay) {
			 ...
			 eventHtml = "<div class=\"" + eventClass + " ui-corner-all\">\
					<div class=\"wc-title" + (isSmallInterval() ? " wc-title-narrow" : "") + "\"></div></div>";
...
// 			 }
      ...
      /*
       * find the weekdays in the current calendar that the calEvent falls within
       */
      _findWeekDaysForEvent : function(calEvent, $weekDayColumns) {
         var event_start = calEvent.start.getTime();
         var event_end = calEvent.end.getTime();

         var $weekDays = [];
         $weekDayColumns.each(function() {
            var day_start = $(this).data("startDate").getTime();
            var day_end = $(this).data("endDate").getTime();
//             if ($(this).data("startDate").getTime() <= calEvent.start.getTime() && $(this).data("endDate").getTime() >= calEvent.end.getTime()) {
            ...
            if ((event_start >= day_start && event_start <= day_end) ||
               ...
               (day_start >= event_start && day_start <= event_end)) {
               $weekDays.push($(this));
               //return false;
            }
         });
      ...
      _updateEventInCalendar : function (calEvent) {
         ...
         var $weekDayColumns = self.element.find(".wc-time-slots .wc-day-column-inner");
         var $calEvents = self._renderEvent(calEvent, $weekDayColumns);
         
         $calEvents.forEach(function($calEvent) {
            var $weekDay = $calEvent.data("weekDay");
            self._refreshEventDetails(calEvent, $calEvent);
            self._positionEvent($weekDay, $calEvent);
            self._adjustOverlappingEvents($weekDay);
         });
...
//          });
      ...
      _positionEvent : function($weekDay, $calEvent) {
         ...
         var firstHourDisplayed = options.businessHours.limitDisplay ? options.businessHours.start : 0;
         var lastHourDisplayed = options.businessHours.limitDisplay ? options.businessHours.end : 24;
         var firstHourTime = new Date($weekDay.data("startDate").getFullYear(), $weekDay.data("startDate").getMonth(), $weekDay.data("startDate").getDate(), firstHourDisplayed).getTime();
         var lastHourTime = new Date($weekDay.data("startDate").getFullYear(), $weekDay.data("startDate").getMonth(), $weekDay.data("startDate").getDate(), lastHourDisplayed).getTime();
         var startMillis = calEvent.start.getTime() - firstHourTime;
         startMillis = startMillis > 0 ? startMillis : 0;
         var eventMillis = Math.min(lastHourTime, calEvent.end.getTime()) - Math.max(firstHourTime, calEvent.start.getTime());
         var pxTop = pxPerMillis * startMillis;
         var pxHeight = pxPerMillis * eventMillis;
         $calEvent.css({top: pxTop, height: pxHeight});
         $calEvent.data("top", pxTop);
         $calEvent.data("bottom", pxTop + pxHeight);
      },
      ...
      _getEventOffsetFromPositionedEventElement : function($weekDay, $calEvent, top, bottom) {
         var options = this.options;
         var $original_weekDay = $calEvent.data("weekDay");
         var original_top = $calEvent.data("top");
         var original_bottom = $calEvent.data("bottom");
         
         var startOffsetMillis = options.businessHours.limitDisplay ? options.businessHours.start * 60 * 60 * 1000 : 0;
         var start = new Date($weekDay.data("startDate").getTime() + startOffsetMillis + Math.round(top / options.timeslotHeight) * options.millisPerTimeslot);
         var end = new Date(start.getTime() + ($calEvent.height() / options.timeslotHeight) * options.millisPerTimeslot);
         var original_start = new Date($original_weekDay.data("startDate").getTime() + startOffsetMillis + Math.round(original_top / options.timeslotHeight) * options.millisPerTimeslot);
         var original_end = new Date($original_weekDay.data("startDate").getTime() + startOffsetMillis + Math.round(original_bottom / options.timeslotHeight) * options.millisPerTimeslot);

         return {start: start.getTime() - original_start.getTime(), end: end.getTime() - original_end.getTime()};
      },
      
      _getEventOffsetFromPositionOffset : function(day_offset, original_top, original_bottom, top, bottom) {
         var options = this.options;
         
         var dayOffsetMillis = day_offset * 24 * 60 * 60 * 1000;
         var startOffsetMillis = options.businessHours.limitDisplay ? options.businessHours.start * 60 * 60 * 1000 : 0;
         var start = new Date(dayOffsetMillis + startOffsetMillis + Math.round(top / options.timeslotHeight) * options.millisPerTimeslot);
         var end = new Date(dayOffsetMillis + startOffsetMillis + Math.round(bottom / options.timeslotHeight) * options.millisPerTimeslot);
         var original_start = new Date(startOffsetMillis + Math.round(original_top / options.timeslotHeight) * options.millisPerTimeslot);
         var original_end = new Date(startOffsetMillis + Math.round(original_bottom / options.timeslotHeight) * options.millisPerTimeslot);
      ...
      _addDraggableToCalEvent : function(calEvent, $calEvent) {
      	 var self = this;
         var options = this.options;
         var originalCalEvent = calEvent;
         var originalLeft;
         var originalStart, originalEnd;
         var last_start_offset, last_end_offset;
         var $weekDayColumns = self.element.find(".wc-day-column-inner");
			 $calEvent.draggable({
				...
				start : function(event, ui) {
				   originalCalEvent = calEvent;
				   originalStart = calEvent.start.getTime();
        		   originalEnd = calEvent.end.getTime();
        		   originalLeft = ui.helper.position().left;
        		   
        		   last_start_offset = 0;
        		   last_end_offset = 0;
        		   
				   var newCalEvent = $.extend(true, {}, originalCalEvent);   		   
        		   
        		   // hide the original event views
        		   originalCalEvent.views.forEach(function($calEvent){
					  $calEvent.hide();
				   });
				   
				   // render new event views
				   $newEvents = self._renderEvent(newCalEvent, $weekDayColumns);
					   
				   // adjust overlap for all columns
				   $weekDayColumns.each(function(){
					  self._adjustOverlappingEvents($(this));
				   });
				...
				drag : function(event, ui) {
				   var oldCalEvent = calEvent;
				   var day_offset = Math.round((ui.position.left - originalLeft)/$calEvent.parent().width());
				   								   
				   var top = Math.round(parseInt(ui.position.top));
				   var bottom = Math.round(parseInt(ui.position.bottom));
				   var original_top = Math.round(parseInt(ui.originalPosition.top));
				   var original_bottom = Math.round(parseInt(ui.originalPosition.bottom));
				   
				   var eventOffset = self._getEventOffsetFromPositionOffset(day_offset, original_top, original_bottom, top, bottom);
				   if (eventOffset.start != last_start_offset || eventOffset.end != last_end_offset) {
 				       last_start_offset = eventOffset.start;
 				       last_end_offset = eventOffset.end;

					   var newCalEvent = $.extend(true, {}, oldCalEvent);
						newCalEvent.start.setTime(originalStart + eventOffset.start);
						newCalEvent.end.setTime(originalEnd + eventOffset.start);
					   
					   // hide old event views
					   oldCalEvent.views.forEach(function($calEvent){
						  $calEvent.hide();
					   });
					   ...
					   setTimeout(function() {
						  oldCalEvent.views.forEach(function($calE){
							 $calE.remove();
						  });
					   }, 100);
 				...
 				stop : function(event, ui) {
				   ...
				   setTimeout(function() {
 				   ...
 				   if (!options.readonly) {
					   calEvent.views.forEach(function($calEvent) {
							self._addDraggableToCalEvent(calEvent, $calEvent);
					   });
				   }
				   
				   // trigger drop callback
				   options.eventDrop(calEvent);
 				}
			 });
      },
...
//                var top = Math.round(parseInt(ui.position.top));
//                var bottom = Math.round(parseInt(ui.position.bottom));
//                var eventOffset = self._getEventOffsetFromPositionedEventElement($weekDay, $calEvent, top, bottom);
//                var calEvent = $calEvent.data("calEvent");
//                var newCalEvent = $.extend(true, {}, calEvent);
//                newCalEvent.start.setTime(calEvent.start.getTime() + eventOffset.start);
//                newCalEvent.end.setTime(calEvent.end.getTime() + eventOffset.start);
//                
//                // hide old event views
//                calEvent.views.forEach(function($calEvent){
//                   $calEvent.hide();
...
//          });
       ...
       */
      _addResizableToCalEvent : function(calEvent) {
         var self = this;
         var options = this.options;
         var originalCalEvent = calEvent;
         var originalStart, originalEnd;
         ...
         var $calEvents = calEvent.views;
         var $calEvent = $calEvents[$calEvents.length-1];
         if ($calEvent) {
			 var $handle = $('<div class="wc-resizable-handle-s">').html("=");
			 $handle.data("calEvent", calEvent);
			 $calEvent.append($handle);
			 $handle.draggable({
//   				containment: ".wc-scrollable-grid",
				revert: 'valid',
				opacity: 0.5,
				...
				drag : function(event, ui) {
				   var oldCalEvent = calEvent;
				   var day_offset = Math.round(ui.position.left/$calEvent.parent().width());
				   ...
				   var eventOffset = self._getEventOffsetFromPositionOffset(day_offset, original_top, original_bottom, top, bottom);
				   var interval = originalEnd + eventOffset.start - originalStart;
				   if (Math.abs(interval) < 30 * 60 * 1000)	return;
				   if (eventOffset.start != last_start_offset || eventOffset.end != last_end_offset) {
 				       last_start_offset = eventOffset.start;
 				       last_end_offset = eventOffset.end;
					   ...
					   var newCalEvent = $.extend(true, {}, oldCalEvent);
					   if (interval > 0) {
					  		newCalEvent.start.setTime(originalStart);
					   		newCalEvent.end.setTime(originalStart + interval);
					   } else {
					   		newCalEvent.start.setTime(originalStart + interval);
					   		newCalEvent.end.setTime(originalStart);
					   }
