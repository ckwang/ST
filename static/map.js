...
superTripperApp.controller('MapCtrl', function($scope) {
  // View
  ...
  var googleRed = "FF7D72";
  var googleGreen = "67BF4B";
  var googleBlue = "6991FD";
  var defaultLetter = "%E2%80%A2";
  ...
  // color in HEX form
  var makeMarkerIcon = function(letter,color,shadow,scale) {
    var icon = {};
  ...
  var getMarkerIconURL = function(order) {
    var letter = String.fromCharCode('A'.charCodeAt() + order);
    var icon = makeMarkerIcon(letter, googleBlue , true,1);
  ...
  var updateResults = function() {
    ...
    if (results) {
      ...
      results.forEach(function(result) {
        ...
        var markerHighLight = function(i) {
          return function() {
            markers[i].setIcon(iconURL["red_pin"]);
        ...
        var markerUnhighLight = function(i) {
          return function() {
            markers[i].setIcon(iconURL["red_measle"]);
          }
        };
        ...
        createEvent_btn.click(function() {
          showAddEvent({"location_names": [result.name],
    ...
    Permission.setElementPermission($scope.dataPermission);
  }
  ...
  var getPixelPositionFromLatLng = function(position) {
    return  projection.fromLatLngToContainerPixel(position);
  }
  ...
  var showEventWindow = function(event, eventMarker) {
    return function(){
      ...
      var content = $('<div class="event-popover-content">').html(getEventContent(event));
      dummy.append(title);
  ...
  var showEventMarkers = function(eventList) {
    // error prevention
    if(!eventList)
      eventList = $scope.dataEvents;
  ...
  var emptyEventMarkers = function(){
    eventMarkers = {};
  };

  // helper function to move the center of map to a specific event location
  var panToEventMarker = function(id) {
    map.panTo(eventMarkers[id].position);
    highlightEventMarker(id);
  };

  var highlightEventMarker = function(id) {
    //eventMarkers[id].setAnimation(google.maps.Animation.BOUNCE);
    var icon = makeMarkerIcon(eventMarkers[id].letter, googleBlue , true,1.5);
    eventMarkers[id].setIcon(icon.pinImage);
    eventMarkers[id].setShadow(icon.pinShadow);

    google.maps.event.trigger(eventMarkers[id], 'click');
  }
  var unhighlightEventMarker = function(id) {
    //eventMarkers[id].setAnimation(null);
    var icon = makeMarkerIcon(eventMarkers[id].letter, googleBlue , true, 1);
    eventMarkers[id].setIcon(icon.pinImage);
    eventMarkers[id].setShadow(icon.pinShadow);
  ...
  var showDetailDirections = function(index, panel) {
//    var panel = $("<div>");

//    highlightRenderer.setDirections( piecewiseDirResult[index] );
    highlightRenderer.setPanel(panel.get(0));
//    return panel;
  }

  var highlightRoute = function(index) {
    highlightRenderer.setDirections(piecewiseDirResult[index]);
    highlightRenderer.setMap(map);
  }
  var unhighlightRoute = function(index) {
    highlightRenderer.setDirections({routes:[]});
    highlightRenderer.setMap(map);
  }

  var toggleOptimalPath = function(){
    dirRenderer.setDirections(optimalPath);
    //dirRenderer.setMap(map);
  }

  var untoggleOptimalPath = function(){
    dirRenderer.setDirections(dirResult);
  }

  // Model
  $scope.data = {};  //stores timelist and distlist

  // Google Map API Components
  var map = null;             // Google Map
  var placesService = null;   // Place Search
  var autocomplete = null;    // Search autocomplete
  var dms = null;             // Get Distance and Time between points
  var dirService = null;      // Get directions between points
  var dirRenderer = null;     // Render route on the map
  var projection = null;      // Projection object from map to pixel
  var iw = null;              // info window on google map
  ...
  var initialize = function() {
    ...
    // render directions
    dirRenderer = new google.maps.DirectionsRenderer({draggable:false, suppressMarkers:true, preserveViewport:true});
    highlightRenderer = new google.maps.DirectionsRenderer({draggable:false, suppressMarkers:true, preserveViewport:true, polylineOptions:{strokeColor:'FF0000', strokeOpacity:0.9, strokeWeight:5} });

    $('#searchTextField').keyup(function(e) {
      // Press enter
      if (e.keyCode === 13) {
        queryCache = $('#searchTextField').val();
        search();
      }
    });
    ...
    $('#showRouteButton').toggle(enableRouteRendering, disableRouteRendering);

    // Whenever event list changes, call these functions

    $scope.$watch("dataEvents", function(value) {
      showEventMarkers(value);
      getEventRoute(value);
      initialPan();
    });
//    Model.Event.bind(showEventMarkers);
...
//    Model.Event.bind(initialPan);
  ...
  var initialPan = function() {
    if(firstPageLoad){
      var eventList = $scope.dataEvents;
      if (eventList && eventList.length && eventList.length !== 0){
        var midIndex = Math.floor(eventList.length/2);
        map.panTo(new google.maps.LatLng(eventList[midIndex].location_lats[0], eventList[midIndex].location_lngs[0]));
      }
      firstPageLoad = false;
    }
  }
  ...
  var initializeGoogleMap = function() {
    ...
    google.maps.event.addListener(map,'center_changed',closeEventWindow);
  ...
  var tilesLoaded = function() {
    ...
    google.maps.event.addListener(map, 'draggend', function(){search();});
    google.maps.event.addListener(map, 'zoom_changed', function(){search();});
  ...
  function getDetails(result, i) {
    return function() {
      ...
      }, showInfoWindow(markers[i]));
    }
  }


  var getEventRoute = function(eventList) {
    dirResult.routes = [];
    piecewiseDirResult = [];
      ...
      if (index === end -1){
        return function(result, status) {
          if (status === google.maps.DirectionsStatus.OK) {
            piecewiseDirResult.push(result);
            ...
            if (dirResult.routes.length === 0){
              dirResult.routes.push({});
              dirResult.routes[0].bounds = result.routes[0].bounds;
              dirResult.routes[0].copyrights = result.routes[0].copyrights;
              dirResult.routes[0].legs = result.routes[0].legs.slice();
              dirResult.routes[0].overview_path = result.routes[0].overview_path.slice();
              dirResult.routes[0].warnings = result.routes[0].warnings.slice();
              dirResult.routes[0].waypoint_order = [];
            }
            else{
              dirResult.routes[0].bounds  = result.routes[0].bounds.union(dirResult.routes[0].bounds);
              dirResult.routes[0].copyrights = result.routes[0].copyrights;
              dirResult.routes[0].legs.push(result.routes[0].legs[0]);
              dirResult.routes[0].overview_path = dirResult.routes[0].overview_path.concat(result.routes[0].overview_path);
              dirResult.routes[0].warnings = dirResult.routes[0].warnings.concat(result.routes[0].warnings);
              dirResult.routes[0].waypoint_order.push(dirResult.routes[0].waypoint_order.length);
            }
        ...
        return function(result, status) {
          ...
          dirService.route(routeQuery, routeCallbackGen(index + 1));
        };
    ...
    dirService.route(routeQuery, routeCallbackGen(1));
